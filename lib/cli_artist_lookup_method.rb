        ...
        artistname = prompt.ask("Do you know their name? We'll check it against our database of artists!") {|q| q.modify :capitalize}
            ...
            if Artist.all.find_by name: artistname
                ...
                if make_appt == true
                        ...
                        prompt.ask("Sweet, thanks #{Client.current_client_instance.name}! We're gonna run it back to you so you can let us know if there's anything we missed:")
                        prompt.ask("Your name is #{Client.current_client_instance.name}, and your artist's name is #{whats_their_name.name}...")
                       # binding.pry
                        ...
                        #binding.pry
                        Appointment.create(client: Client.all.last, date: date, description: description, artist:  whats_their_name, tattoo_shop: whats_their_name.tattoo_shop)
                        end
                ...
                prompt.ask("We dont know 'em! Let's start again")
                make_an_appointment
            end
